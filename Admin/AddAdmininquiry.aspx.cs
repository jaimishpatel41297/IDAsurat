...
public partial class Admin_AddAdmininquiry : System.Web.UI.Page
{
    Connection D = new Connection();
    HttpCookie cookie;
    SqlConnection CS = new SqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings["IDASurat"].ConnectionString.ToString());
    ...
    private void BindDDL()
    {
        try
        {
            ...
            DataTable dtss = D.GetDataTable("select * from EmployeeMaster");
    ...
    private void BindData()
    {
        if (Request.Cookies["AdminId"] != null)
        {
            ...
            try
            {
                ...
                DataTable dt = D.GetDataTable("select distinct im.*,c.Title as CName,cm.Amount as CAmount from InquiryMaster im,Course c,CourseMaster cm where im.CourseId=c.Id and im.CourseId=cm.Courseid and UserId= 0  order by id desc");
                if (dt.Rows.Count > 0)
                {
                    gv.DataSource = dt;
                    gv.DataBind();
    ...
    protected void lbEdit_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                txtdob.Text = dt.Rows[0]["DOB"] != System.DBNull.Value ? Convert.ToDateTime(dt.Rows[0]["DOB"].ToString()).ToString("yyyy-MM-dd") : "";
